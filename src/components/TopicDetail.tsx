...
export function TopicDetail() {
  ...
  const dayGroups = useMemo(() => {
    const groups = new Map<number, typeof allTopicWords>();
    for (const w of allTopicWords) {
      const arr = groups.get(w.day) || [];
      arr.push(w);
      groups.set(w.day, arr);
    }
    return [...groups.entries()].sort((a, b) => a[0] - b[0]);
  }, [allTopicWords]);

  const testedCount = allTopicWords.filter(w => wordStates[w.id]).length;
  const masteredCount = allTopicWords.filter(w => wordStates[w.id]?.box === 6).length;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      ...
      <p style={{ fontSize: "13px", color: "var(--color-ink-muted)", margin: 0 }}>
        {allTopicWords.length} words · {testedCount} tested · {masteredCount} mastered
      </p>

      {/* Words grouped by day */}
      {dayGroups.map(([day, words]) => (
        <div key={day}>
          <div style={{
            fontSize: "12px", fontWeight: 700, color: "var(--color-ink-muted)",
            textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: "6px",
            display: "flex", alignItems: "center", gap: "8px",
          }}>
            Day {day}
            <span style={{ fontSize: "10px", fontWeight: 500, color: "var(--color-ink-faint)" }}>
              ({words.length} words)
            </span>
          </div>
              ...
              return (
                <div key={word.id} style={{
                  ...
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "12px" }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: "15px", fontWeight: 600, marginBottom: "2px" }}>{word.german}</div>
                      <div style={{ fontSize: "13px", color: "var(--color-ink-muted)" }}>{word.english}</div>
                    </div>
                    {boxColors ? (
                      <span style={{
